...
function () {
  ...
  _createClass(Game, [{
    ...
    value: function move(keyCode) {
      ...
      activeTank.position().left, parent.position().left + widthPar);

      if (activeTank.position().left >= parent.offset().left || activeTank.position().left <= parent.offset().left + widthPar) {
        // console.log($(`.gameZone`).offset().left , $(`.active`).position().left);
        if (keyCode === 37) {
          this.position -= speed;
